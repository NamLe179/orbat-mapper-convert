 ...
 * Compatible with Next.js (Client & Server/Edge Runtimes).
 *
...
export async function encryptScenario(
  ...
  const key = await deriveKey(password, salt as unknown as BufferSource, DEFAULT_ITERATIONS);
...
export async function decryptScenario(
  ...
  const key = await deriveKey(password, salt as unknown as BufferSource, encrypted.crypto.iterations);
...
function bufferToBase64(buffer: ArrayBuffer | Uint8Array): string {
  ...
  for (let i = 0; i < bytes.length; i += CHUNK_SIZE) {
    ...
    // Sử dụng spread operator hoặc apply cẩn thận với TypeScript
    binary += String.fromCharCode.apply(null, chunk as unknown as number[]);
  }
  // btoa có sẵn trong global scope của trình duyệt và Node.js hiện đại
  return btoa(binary);
}

function base64ToUint8Array(base64: string): Uint8Array {
  // atob có sẵn trong global scope
