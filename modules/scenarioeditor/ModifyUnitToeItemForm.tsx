"use client";

import React, { useMemo, useEffect, useCallback } from "react";
import { klona } from "klona";

// Types
import type {
  EUnitEquipment,
  EUnitSupply,
  NUnitEquipment,
  NUnitPersonnel,
} from "@/types/internalModels";
import { type ToeEditStore } from "@/stores/toeStore";

// Hooks & Utils
import { useActiveScenario } from "@/components/injects";
import { useForm } from "@/hooks/forms";
import { useTimeFormatters } from "@/stores/timeFormatStore";

// Components
import InputGroup from "@/components/InputGroup";
import InputCheckbox from "@/components/InputCheckbox";
import FormFooter from "@/modules/scenarioeditor/FormFooter";

type FormValues = NUnitEquipment | NUnitPersonnel;

interface Props {
  itemData: EUnitEquipment | EUnitSupply;
  editStore: ToeEditStore;
  onCancel: () => void;
  onDiffOnHand: (form: FormValues) => void;
  onUpdateOnHand: (form: FormValues) => void;
  onUpdateCount: (form: FormValues) => void;
}

export default function ModifyUnitToeItemForm({
  itemData,
  editStore: editStoreProp,
  onCancel,
  onDiffOnHand,
  onUpdateOnHand,
  onUpdateCount,
}: Props) {
  // --- Context & Stores ---
  const { time } = useActiveScenario();
  const fmt = useTimeFormatters();
  const editStore = editStoreProp as any; // Type assertion to fix union type inference

  // --- Form Logic ---
  const { form, setForm } = useForm<FormValues>({
    ...itemData, // Khởi tạo với dữ liệu hiện tại
  } as FormValues);

  // --- Watchers (useEffect) ---

  // Tương đương watch props.itemData immediate
  useEffect(() => {
    setForm(klona(itemData) as FormValues);
  }, [itemData]);

  // Reset form khi thay đổi mode (On-hand mode hoặc Diff mode)
  useEffect(() => {
    setForm(klona(itemData) as FormValues);
  }, [editStore.isOnHandMode, editStore.isDiffMode, itemData]);

  // --- Computed (useMemo) ---
  const formattedTime = useMemo(() => {
    return fmt.scenarioFormatter.format(Number(time.getScenarioTime()));
  }, [fmt.scenarioFormatter, time]);

  // --- Handlers ---
  const onSubmit = useCallback(
    (e?: React.FormEvent | React.KeyboardEvent) => {
      if (e) {
        // Ngăn submit đúp khi Enter trong input
        if (
          (e as any).nativeEvent instanceof KeyboardEvent &&
          e.target instanceof HTMLInputElement
        ) {
          return;
        }
        if ("preventDefault" in e) e.preventDefault();
      }

      if (editStore.isOnHandMode) {
        if (editStore.isDiffMode) {
          onDiffOnHand({
            id: form.id,
            onHand: editStore.diffValue,
            count: -1,
          } as FormValues);
        } else {
          onUpdateOnHand({
            id: form.id,
            onHand: form.onHand,
            count: -1,
          } as FormValues);
        }
      } else {
        onUpdateCount({ id: form.id, count: form.count } as FormValues);
      }
    },
    [editStore, form, onDiffOnHand, onUpdateCount, onUpdateOnHand]
  );

  // --- Keyboard Shortcuts ---
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") onCancel();
      if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) onSubmit();
    };
    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [onCancel, onSubmit]);

  return (
    <form onSubmit={onSubmit}>
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold">{itemData.name}</h3>
        <div className="flex items-center gap-1"></div>
      </div>

      <div className="mt-4">
        <InputCheckbox
          checked={editStore.isOnHandMode}
          onCheckedChange={(val) => editStore.setIsOnHandMode(!!val)}
          label={`Edit item at ${formattedTime}`}
          description=""
        />
      </div>

      <section className="mt-4 grid grid-cols-2 items-start gap-6">
        <InputGroup
          label="Initial value"
          type="number"
          disabled={editStore.isOnHandMode}
          value={form.count}
          onChange={(e) => setForm({ ...form, count: Number(e.target.value) })}
          min="0"
          autoFocus={!editStore.isOnHandMode}
        />
        
        <InputGroup
          label="Available / On hand"
          type="number"
          disabled={!editStore.isOnHandMode || editStore.isDiffMode}
          value={(form as any).onHand}
          onChange={(e) => setForm({ ...form, onHand: Number(e.target.value) } as FormValues)}
          min="0"
          autoFocus={editStore.isOnHandMode && !editStore.isDiffMode}
        />

        {editStore.isOnHandMode && (
          <>
            <InputCheckbox
              label="Add/subtract mode"
              description=""
              checked={editStore.isDiffMode}
              onCheckedChange={(val) => editStore.setIsDiffMode(!!val)}
            />
            {editStore.isDiffMode && (
              <InputGroup
                label="Add/subtract"
                type="number"
                autoFocus={editStore.isDiffMode}
                value={editStore.diffValue}
                onChange={(e) => editStore.setDiffValue(Number(e.target.value))}
              />
            )}
          </>
        )}
      </section>

      <FormFooter onCancel={onCancel} showNextToggle />
    </form>
  );
}