...
export class OrbatChart {
  ...
  /**
   * Cleans up event listeners and panzoom instance.
   * Call this in the useEffect cleanup function in React.
   */
  cleanup() {
    if (this.svg) {
  ...
  private _addPanZoomInteraction() {
    ...
    this.pz = Panzoom(this.svg.node()!, {
      maxScale: 10,
      pinchAndPan: true,
      // canvas: true, // Optional: might perform better depending on DOM size
    });
    
    this.svg.node()?.parentElement?.addEventListener("wheel", (event: WheelEvent) => {
      if (!event.ctrlKey) return; // Optional UX: only zoom on ctrl+wheel to allow scrolling
  ...
  highlightLevel(levelNumber: number) {
    ...
    // React Migration: Scoped selection to this.svg instead of global select()
    // to prevent conflicts if multiple charts are on the same page.
    const backgroundLayer = this.svg.select("#o-highlight-layer");
    const groupElement = this.svg.select(`#o-level-${levelNumber}`) as GElementSelection;
  ...
  highlightGroup(renderedBranch: RenderedBranch) {
    ...
    // React Migration: Scoped selection
    const backgroundLayer = this.svg.select("#o-highlight-layer");
    ...
    if (this.options.onBranchClick) {
      tmp.on("click", (e) => {
        this.options.onBranchClick?.(
          renderedBranch.units[0]?.parent?.unit.id || "0", // Ensure ID is string/number as per type
  ...
  private _createSvgRootElement(
    ...
    const svg = select(parentElement)
      ...
      .attr("class", "orbat-chart"); // Consider using Tailwind classes in wrapper instead
      
    if (elementId) svg.attr("id", elementId);

    // In Next.js/React, styles are typically CSS modules or Tailwind.
    // However, for generated SVG, creating an internal style block is valid.
  ...
  private _renderLevel(
    ...
    // Use closures to keep access to scope variables (D3 pattern)
    // NOTE: In strict FP/React style, these might be pure functions, 
    // but here we are mutating the chart state/DOM which is typical for D3 classes.
    switch (levelLayout) {
