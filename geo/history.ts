...
function createGreatCircleArcFeature(leg: Position[]) {
  ...
  for (let i = 0; i < leg.length - 1; i++) {
    ...
    if (distance > 100000) {
      ...
      // @ts-ignore: Turf vs GeoJSON types
...
interface CreateUnitPathFeaturesOptions {
  isEditMode?: boolean;
  timeZone?: string;
  // REFACTOR: Pass formatter explicitly instead of using hook inside
  trackFormatter?: { format: (t: number) => string }; 
}
...
export function createUnitPathFeatures(
  ...
  const { 
    isEditMode = false, 
    // Default formatter if not provided (mock)
    ...
    const f = new Feature({
      ...
      label:
        state.t > Number.MIN_SAFE_INTEGER
          ? `#${index} ${trackFormatter.format(state.t)}`
          : `#${index}`,
    });
    f.setId(state.id);
    return f;
