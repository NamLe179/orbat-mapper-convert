...
export function parseOdinDragonTemplate(
  ...
  // Helper logic for templates needs to handle a single root usually, 
  // but keeping structure similar to main parser for safety
  ...
  function helper(units: Unit[]) {
    units.forEach((unit) => {
      // Guard clause if unit is undefined (e.g. template parsing failed for root)
      if (!unit) return;
...
function convertUnitTemplateRowToUnit(
  ...
  if (includeEquipment || includePersonnel) {
    ...
    // Update parentUIDs to include children equipment for personnel check?
    // Original logic seemed to reset/update parentUIDs for nested checks
