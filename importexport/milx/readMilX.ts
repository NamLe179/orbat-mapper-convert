...
/**
 * Parses MilX XML structure into internal Layer definitions.
 * Requires DOM APIs (Client-side only).
 */
...
function convertProperties(f: MilXSymbolProperties): ImportGeoJsonProperties {
  ...
  for (const key of supportedAttributes) {
    if (key in f) {
      //@ts-ignore: Dynamic access based on defined map
...
function convertColor(milxColor: string): string {
  // MilX often uses ARGB format, this converts to Hex #RRGGBB
  // Assumes format like #AARRGGBB or AARRGGBB
