...
export function useUnitManipulations(store: NewScenarioStore) {
  // Use a fallback for groupUpdate if not implemented in the basic store wrapper
  const { update, groupUpdate = (fn: any) => fn() } = store;
  ...
  function updateUnit(
    unitId: EntityId,
    data: UnitUpdate,
    { doUpdateUnitState = false, ignoreLocked = false, s = store.state, noUndo = false } = {},
  ) {
    ...
    if (noUndo) {
      ...
      update((draftState) => {
        const u = draftState.unitMap[unitId];
        if (!u) return;
        Object.assign(u, { ...data });
        draftState.unitMap[unitId] = klona(u);
      });
    }
  ...
  function updateUnitLocked(unitId: EntityId, locked: boolean) {
    // This looks like a direct mutation without update/draft?
    // It should probably be inside update if it's meant to be reactive
    // But keeping original logic structure:
    const unit = store.state.unitMap[unitId];
    if (!unit) return;
    // We should strictly use update for store mutation
    update(s => {
  ...
  function changeUnitParent(
    ...
    update((s) => {
      ...
      if (target === "above" || target === "below") {
        parentId = getUnitOrSideGroup(targetId, s)?._pid!;
      }
      ...
      if (parents.some(p => p.id === unit.id)) { // Fix comparison
  ...
  function walkSubUnits(
    ...
    const { state: s = store.state, includeParent = false } = options;
  ...
  function walkSide(sideId: EntityId, callback: NWalkSideCallback, s = store.state) {
    let level = 0;
    const side = s.sideMap[sideId];
    if (!side) return;
  ...
  function walkItem(itemId: EntityId, callback: NWalkSideCallback, s = store.state) {
    let level = 0;
  ...
  function getUnitOrSideGroup(id: EntityId, s = store.state): NUnit | NSideGroup | undefined {
  ...
  function addUnit(
    ...
    parentId: EntityId,
    index?: number,
    { noUndo = false, updateState = false, s = store.state } = {},
  ): EntityId {
    ...
    if (noUndo) {
      ...
      // but in Zustand/Immer pattern we should be careful mixing access.
      // If s = store.state (default), we shouldn't mutate it directly outside update.
      ...
      update((draftState) => {
        draftState.unitMap[unit.id] = unit;
        let parent = getUnitOrSideGroupOrSide(unit._pid!, draftState);
        if (!parent) return;
        if (index === undefined) {
          parent.subUnits.push(unit.id);
        } else {
  ...
  function createSubordinateUnit(parentId: EntityId, data: Partial<NUnit> = {}) {
    ...
    if (data.sidc) {
      ...
      const side = "_pid" in parent ? store.state.sideMap[parent._pid] : parent;
    ...
    // Open parent
    update(s => {
        const p = getUnitOrSideGroupOrSide(parentId, s);
        if(p) p._isOpen = true;
    });
    return newUnitId;
  }
  ...
  function cloneUnit(
    ...
    let parent = getUnitOrSideGroup(unit._pid, store.state);
  ...
  function expandUnit(unit: NUnit): Unit {
    return {
      ...unit,
      state: [],
      subUnits: unit.subUnits.map((subUnitId) => expandUnit(store.state.unitMap[subUnitId])),
      equipment: unit.equipment?.map(({ id, count, onHand }) => ({
        name: store.state.equipmentMap[id].name || "",
        count,
        onHand,
      })),
      personnel: unit.personnel?.map(({ id, count, onHand }) => ({
        name: store.state.personnelMap[id].name || "",
        count,
        onHand,
      })),
      supplies: unit.supplies?.map(({ id, count, onHand }) => ({
        name: store.state.supplyCategoryMap[id].name || "",
        count,
        onHand,
      })),
    };
  ...
  function expandUnitWithSymbolOptions(unit: NUnit): Unit {
    return {
      ...
      subUnits: unit.subUnits.map((subUnitId) =>
        expandUnitWithSymbolOptions(store.state.unitMap[subUnitId]),
      ),
  ...
  function deleteUnitStatus(id: string): boolean {
    ...
    const isUsed = Object.values(store.state.unitMap).some((unit) => {
  ...
  function isUnitLocked(unitId: EntityId, { excludeUnit = false } = {}): boolean {
    ...
    if (excludeUnit) {
      return !!(
        store.state.sideMap[unit._sid]?.locked ||
        (unit._gid && store.state.sideGroupMap[unit._gid]?.locked)
      );
    }
    return !!(
      store.state.sideMap[unit._sid]?.locked ||
  ...
  function isUnitHidden(unitId: EntityId): boolean {
    ...
    return !!(
      store.state.sideMap[unit._sid]?.isHidden ||
      (unit._gid && store.state.sideGroupMap[unit._gid]?.isHidden)
    );
  }

  function convertStateEntryToInitialLocation(unitId: EntityId, index: number) {
    const u = store.state.unitMap[unitId];
  ...
  return {
    ...
    clearUnitState,
    setUnitState,
    // Replaced Vue computed with getter
    get units() { return Object.values(store.state.unitMap); },
    getUnitOrSideGroup,
    getUnitById: (id: EntityId) => store.state.unitMap[id],
    getUnitByName: (name: string) => {
      for (const unit of Object.values(store.state.unitMap)) {
