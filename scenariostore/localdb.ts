 ...
 * Renamed from 'useIndexedDb' to 'getIndexedDb' because this is an async utility function,
 * not a React Hook (React Hooks cannot be async).
 */
export async function getIndexedDb() {
  ...
  async function downloadAsJson(id: string, fileName?: string) {
    ...
    if (!name) {
      // Dynamic import works fine in Next.js/Webpack
      //@ts-ignore
