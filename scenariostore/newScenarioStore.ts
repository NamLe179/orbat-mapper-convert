...
export type ActionLabel = string; // Define purely for the update signature

// Helper to match the interface expected by geo.ts and io.ts
export interface NewScenarioStore {
  state: ScenarioState;
  update: (
    recipe: (draft: ScenarioState) => void,
    meta?: { label: ActionLabel; value?: any }
  ) => void;
  groupUpdate: (
    callback: () => void,
    meta?: { label: ActionLabel; value?: any }
  ...
  onUndoRedo: (
    callback: (payload: { action: "undo" | "redo"; meta: any }) => void
  ) => () => void;
  subscribe: (listener: (state: ScenarioState) => void) => () => void;
  // Expose raw zustand store if needed
  _store: ReturnType<typeof createVanillaStore>; 
}
...
function createVanillaStore(initialState: ScenarioState) {
  return createStore(
    subscribeWithSelector(
      immer<ScenarioState>(() => initialState)
    )
  );
}

/**
 * Creates a new scenario store instance.
 * Adapts Zustand to match the "object with state property" pattern 
 * expected by the rest of the converted logic (geo.ts, io.ts).
 */
export function useNewScenarioStore(data: Scenario): NewScenarioStore {
  ...
  // Undo/Redo listeners placeholder
  // In a real implementation, you would use a middleware like 'zundo'
  ...
  const storeWrapper: NewScenarioStore = {
    ...
    update: (recipe, meta) => {
      ...
      // Here you would push to history stack if you implement undo/redo
      if (meta) {
        // console.log("Action:", meta.label);
      }
    },
    groupUpdate: (callback, meta) => {
        // Trong hệ thống undo/redo đầy đủ, đây sẽ là nơi bắt đầu transaction
        callback();
  ...
  useScenarioTime(storeWrapper).setCurrentTime(storeWrapper.state.currentTime);
